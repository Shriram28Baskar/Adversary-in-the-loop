...
- FR-004a **Zero egress.** The honeypot container has no permitted outbound network path at all; only inbound connections to the attacker-facing SSH port and their return traffic are allowed. This is enforced at the network layer (host firewall rules on the honeypot host; an `internal` network in the single-host profile), not merely by application configuration. Cowrie features that would generate outbound traffic (download/fetch emulation, TCP/port forwarding) are disabled in configuration as defense in depth. *MUST.* Acceptance: an automated test attempts arbitrary outbound TCP/UDP connections from inside the honeypot container (internet address, database, Gateway, Log Shipper) and confirms each is blocked at the network layer; a configuration test confirms forwarding and download features are disabled.
