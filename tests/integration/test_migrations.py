...
Empty database -> head, strict ordering of 0001..0014, presence of every
...
EXPECTED_REVISIONS = [f"{n:04d}" for n in range(1, 15)]
EXPECTED_FILES = [
...
def test_downgrade_each_step_then_reupgrade(pg: Cluster, fresh_db: str) -> None:
    ...
    pg.upgrade(fresh_db, "head")
    assert _rows(pg, fresh_db, "SELECT version_num FROM public.alembic_version") == {"0014"}
