...
**Promotion and reconstruction semantics (engineering decisions D1–D7, ADR-024).** Promotion (`intel_service/promotion.py`, the only writer of `AttackSession`, `AttackEvent`, `AttackerBehavior` and `BehaviorEvent`) re-validates each staged payload with the Log Shipper's parser and handles one session at a time, in one transaction per session:

- **D1 order:** a session's events are numbered `seq = 1..n` by `(occurred_at ASC, raw_record_id ASC)`.
- **D2 trigger:** a session is promoted only once its `session_closed` event is staged; sessions without one stay pending (counted and reported). There is no idle timeout and no wall-clock input.
- **D3 late events:** an event for an already-promoted session is quarantined as `promotion.late_event`; history is never changed.
- **D4 behaviors:** one `AttackerBehavior` per distinct phase in the session; `ordinal` follows the phase's first `seq`, ties broken by `behavior_phase` enum order; `started_at`/`ended_at` are its earliest and latest event times.
- **D5 identifiers:** `AttackSession`, `AttackEvent` and `AttackerBehavior` IDs are UUIDv5 values in fixed repository namespaces, named by the P0 canonical JSON of `(source_type, cowrie_session_id)`, plus `seq` or `ordinal`.
- **D6 session fields:** `src_ip`/`src_port` come from the single `session_connect` event; `first_event_at` and `first_raw_record_id` from `seq = 1`. No connect event → `promotion.no_connect_event`; a second connect event or a differing `src_ip` → `promotion.conflicting_field`. The whole session is quarantined, never partly chosen.
- **D7 text:** `raw_text` is the event's single attacker-supplied field (`input`, `url`, `filename`, `version`, or `username` for logins); connect, close and `other` events carry empty text. Passwords go only to `attempted_secret`. NUL and lone surrogates, which PostgreSQL text cannot store, become U+FFFD in the raw form.

A staged payload that no longer validates is quarantined as `promotion.invalid_payload`.

**Determinism guarantee (exact scope).** Reconstruction is deterministic for a fixed, immutable staging dataset, **including its `raw_record_id` values**. Given the same staged rows (same `raw_record_id`, payload and `ingest_mode`) present when a session is promoted, and the same phase-rule artifact and code, P4 produces identical rows: `seq` values, session, event and behavior identifiers, links, normalized fields, provenance anchors and quarantine outcomes. This holds whatever order the rows were physically inserted, read or listed in. It does **not** hold across different `raw_record_id` assignments. If the same logical events are staged in a different arrival order, they receive different staging IDs, so events with identical `occurred_at` legitimately take different `seq` values and, since event identity is `(session, seq)`, different event IDs (D1 is frozen). Untied events, session identity and phase membership are unaffected. The Log Shipper assigns staging IDs deterministically for a given corpus (files by name, lines in order), which is why re-ingesting the fixture corpus into a fresh database reproduces the reconstruction exactly. Which events a session contains also depends on what has been staged by the time its `session_closed` is processed (D2, D3).

**Operational safety bounds (not behavioral semantics).** Two bounds protect `intel-service` and the shared database. They are not judgements about attacker behavior and they do not change D1–D7.

- **Session event bound** (`max_session_events`, default 10,000). A closed session with more unhandled events than this is not promoted; each of its events is quarantined as `promotion.session_too_large`, auditable per event. It is never partly promoted.
- **Backlog bound** (`max_unhandled_events`, default 200,000 unhandled staged events per run). When exceeded, the run stops with an error before writing anything, including quarantine rows. Events of open sessions stay unhandled (D2) and count toward this bound, so a large number of never-closed sessions can halt promotion until an operator raises the bound or an idle-timeout policy is separately decided.
